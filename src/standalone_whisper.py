...
class StandaloneWhisperApp:
    ...
    def __init__(
        ...
        attention_dim: int,
        mel_filter: Optional[np.ndarray] = None,
    ...
    def _load_or_create_mel_filter(self):
        ...
        try:
            # Look for mel_filters.npz in multiple possible locations
            possible_paths = [
                "mel_filters.npz",  # Current directory
                "../mel_filters.npz",  # Parent directory (when running from backend/)
                os.path.join(os.path.dirname(__file__), "..", "mel_filters.npz"),  # Relative to this file
            ]
            
            mel_filter_path = None
            for path in possible_paths:
                if os.path.exists(path):
                    mel_filter_path = path
                    break
            
            if mel_filter_path:
                # print(f"✅ Loading optimized Whisper mel filters from {mel_filter_path}")
                ...
                if not self.mel_filter_loaded:
                    print("⚠ mel_filters.npz not found in any expected location")
    ...
    def transcribe(
        self, audio: Union[np.ndarray, str], audio_sample_rate: Optional[int] = None
    ) -> str:
        """
